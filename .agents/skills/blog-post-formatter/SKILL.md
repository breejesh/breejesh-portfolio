...
* **No Inline Images:** Do not place inline images within the markdown body of the post. If inline images are needed, the user will add them manually or explicitly ask for them. Store the cover in `public/assets/images/` and reference it with a local absolute path (e.g. `/assets/images/my-image.webp`).
